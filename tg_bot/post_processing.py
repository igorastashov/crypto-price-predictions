...
async def post_processing_data(model_predict, end_date, periods, conf):
    ...
    :param end_date: Последняя временная отметка после которой
    ...
    """
    add_dates = [
        (end_date + timedelta(day)).strftime("%Y-%m-%d") for day in range(periods)
    ]
    predict_df = pd.DataFrame(
        model_predict.values, columns=["prediction"], index=add_dates
    )
...
async def get_data_for_plot(data, back_days, ticker_predict, predict_df):
    """
    Таблица`BACK_DAYS` с пустыми столбцами:
    `prediction`, `left_int`, `right_in`.
    А так же подставленный снизу `predict_df` с предсказаниями на `PERIODS`.

    :param data: загруженные данные с yfinance
     см. `pre_processing.py: data_loader`;
    :param back_days: Последнее кол-во дней для отображения
     на итоговом графике совместно с предсказанием;
    :param ticker_predict: Акция для предсказания;
    :param predict_df: Таблица с предсказаниями.
    :return: Составленная таблица с предыдущими значениями
     и предсказанными + доверительный инетрвал.
    """
